...
RegisterServerEvent('gcphone:GetBanqueData')
AddEventHandler('gcphone:GetBanqueData', function()
    local source = source
    local player = getPlayerID(source)

    MySQL.Async.fetchAll('SELECT * FROM banque_transation WHERE identifier = @id', {['@id'] = player}, function(result)
        TriggerClientEvent('gcphone:ReceiveBanqueData', source, result)
    end)
end)
...
-- App Garage
--====================================================================================
RegisterServerEvent('gcPhone:getGarage')
AddEventHandler('gcPhone:getGarage', function (source)
    local xPlayer = ESX.GetPlayerFromId(source)
	local vehicles  = {}
	...
	MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner', {
		...
		for i=1, #result, 1 do
			table.insert(vehicles, {plate = result[i].plate, garage = result[i].garage, stored = result[i].stored, name = result[i].vehiclename})
		end
